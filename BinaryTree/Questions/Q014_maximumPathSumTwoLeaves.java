...
public class Q014_maximumPathSumTwoLeaves extends BinaryTree.Base {
...
//		System.out.println(root.toString());
	...
	public static int MaxPathSum(TreeNode node, pair p) {
		...
		if (node.left != null && node.right != null) {
			p.res = Math.max(Math.max(left + right + node.val, p.res), max + node.val);

			return max + node.val;
		}

		int rVal = (node.left != null ? left : right) + node.val;
